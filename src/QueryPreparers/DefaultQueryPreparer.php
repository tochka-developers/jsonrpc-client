...
class DefaultQueryPreparer implements QueryPreparer
{
    ...
    public function prepare(string $methodName, array $params, ClientConfig $config): JsonRpcRequest
    {
        ...
        return new JsonRpcRequest($methodName, $inputArguments, Str::uuid());
    }
    ...
    protected function mapMethods(ClientConfig $config): void
    {
        ...
        try {
            ...
            throw new JsonRpcClientException(0, sprintf('Cannot parse proxy class DocBlock: %s', $e->getMessage()));
        }
    ...
    protected function checkType($value, array $argument, string $method): void
    {
        ...
        throw new JsonRpcClientException(
            0,
            sprintf(
                'Error while mapping jsonrpc client method parameter: [%s:%s], expected %s, actual %s',
                $method,
                $argumentName,
                $type,
                gettype($value)
