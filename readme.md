...
* `authType` -- тип HTTP аутентификации на сервере, может быть установлено в `digest`, `ntlm`, `basic`, `gss`. Если аутентификация не используется, этот параметр не указывать.
* `authUsername` -- имя пользователя для HTTP аутентификации на сервере. Используется только в случае установки `authType` в одно из допустимых значений.
* `authPassword` -- пароль пользователя для HTTP аутентификации на сервере. Используется только в случае установки `authType` в одно из допустимых значений.
